...
/*--Tabla intermedia usuario-carpeta--*/
CREATE TABLE user_folders(
    id_user INT UNSIGNED NOT NULL,
    id_folder INT NOT NULL,
    is_owner BOOLEAN NOT NULL DEFAULT TRUE,
    PRIMARY KEY (id_user, id_folder),
    FOREIGN KEY (id_user) REFERENCES group_user.user(id),
    FOREIGN KEY (id_folder) REFERENCES folder(id_folder)
)DEFAULT CHARACTER SET utf8 COLLATE utf8_bin;

/*-- Tabla intermedia usuario-archivo --*/
...
BEGIN 
    ...
    INSERT INTO user_folders(id_user, id_folder, is_owner)
    VALUES (p_id_user, v_id, TRUE);
...
BEGIN
    ...
    INSERT INTO user_files(id_user, id_file, is_owner)
    VALUES (p_id_user, v_id, TRUE);
...
DELIMITER //
CREATE PROCEDURE user_get_folders(
    IN p_id_user INT UNSIGNED
)
BEGIN
    SELECT f.* FROM folder f
    JOIN user_folders uf ON f.id_folder = uf.id_folder
    WHERE uf.id_user = p_id_user;
END //
DELIMITER ;

/*-- Obtener archivos de un usuario--*/
DELIMITER //
CREATE PROCEDURE user_gets_files(
    IN p_id_user INT UNSIGNED
)
BEGIN
    SELECT f.* FROM file f
    JOIN user_files uf ON f.id_file = uf.id_file
    WHERE uf.id_user = p_id_user;
END //
DELIMITER ;

/*--Obtener contenido de una carpeta para un usuario especifico --*/
DELIMITER //
CREATE PROCEDURE user_get_folder_content(
    IN p_id_user INT UNSIGNED,
    IN p_id_folder INT
)
BEGIN
    IF EXISTS (SELECT 1 FROM user_folders
                WHERE id_user = p_id_user AND id_folder = p_id_folder) THEN
        SELECT f.*, 'folder' AS type FROM folder f
        JOIN folder_folders ff ON f.id_folder = ff.id_child_folder
        WHERE ff.id_parent_folder = p_id_folder;

        SELECT f.*, 'file' AS type FROM file f
        JOIN folder_files ff ON f.id_file = ff.id_file
